...
class API:
    ...
    def CallRegisterAPI(self, name : str, family : str, gender : str, country : str, *, status : int):
        ...
        if req.status_code == 200:
            ...
            if res_status == RegisterAPIStatus.Succesfull:
                SaveAccountInfo(phone_number, name, family, gender, country, status, res_status)
                return True
            return False
        raise FaildAPIConnection() # Error when can't connect to Membersgram api

    def SaveAccountInfo(self, *args):
